  ...
  describe("after adding a few members", () => {
    ...
    describe("after calling removeLastMember as a member", () => {
      ...
      it("It should pop off the most recent club member", async () => {
        assert(
          !(await contract.isMember(accounts[2])),
          "expected the most recently added member to be removed"
        );
      });
      ...
      describe("after calling removeLastMember twice", () => {
        ...
        it("It should pop off the most recent club member", async () => {
          assert(
            !(await contract.isMember(accounts[1])),
            "expected the most recently added member to be removed"
          );
        });
      });
...
async function assertRevert(promise: Promise<any>, msg: string) {
  let ex;
  try {
    await promise;
  } catch (_ex) {
